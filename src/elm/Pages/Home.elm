...
histogram lst =
    let 
        ...
        pcHeight n = String.fromFloat (toFloat n / norm * size) ++ "rem"
        
        viewPair (st, n) = div [ class "flex flex-col justify-end align-center m-1.5 text-sm text-center" ] 
                [ div [] [ text (String.fromInt n) ]  
                , div [ class "bg-secondary hover:bg-accent rounded-sm w-5 h-full", style "height" (pcHeight n) ] [ text " "]
                , div [] [ text st ] 
                ]

        items = List.map viewPair lst
    in 
    div [ class "flex justify-center" ] items 


histEx = [ ("V0", 8), ("V1", 10), ("V2", 14), ("V3", 9), ("V4", 12), ("V5", 4)]



boulderDetail = 
    let 
        viewItem idx { name, grade } = div [ class "flex px-4 py-3 items-center border-t-2 first:border-0 border-focus"]
            [ Ui.counter Ui.Color.Secondary (idx + 1) 
            , div [ class "mx-4 font-bold grow" ] [text (name ++ " (" ++ grade ++ ")") ]
            , div [ class "font-bold text-xl grow-0"] [ text "+"]
            ]

        items = List.indexedMap viewItem itemsEx
    in
    div [] 
        [ div [ class "mx-4 my-3" ] 
...
carousel imgs = 
    let
        size = List.length imgs

        toID idx = "carousel-slide" ++ String.fromInt (remainderBy size idx)
        
        viewItem idx url = div [ class "carousel-item relative w-full", id (toID idx) ]
